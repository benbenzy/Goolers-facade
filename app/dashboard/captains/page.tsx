...
function CaptainsPage() {
  ...
  const { data: captains } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('captains')
        .select(
          '*,players(*),teams(*,counties(name),constituencies(name),wards(name))'
        );
      if (error) {
  ...
  const {
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['captains'] }),
  });
  return (
    <div className="bg-slate-800 rounded-md p-5 mt-5">
      <div className="flex flex-row items-center justify-between">
        ...
        <Link href={'/dashboard/captains/new'}>
          ...
          {Array.isArray(captains) &&
            captains?.map((item: any) => (
              <tr key={item?.id} className="m-5 hover  ">
                ...
                <td>{item?.players?.phone}</td>
                <td>{item?.teams?.counties?.name}</td>
                <td>{item?.teams?.constituencies?.name}</td>
                <td>{item?.teams?.wards?.name}</td>
                <td>
                  <button
                    onClick={() => {
