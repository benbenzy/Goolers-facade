'use client';
import { CaptainFormPost } from '@/app/types/types.d';
...
interface props {
  submit: SubmitHandler<CaptainFormPost>;
}

const AddCaptainForm: FC<props> = ({ submit }) => {
  const supabase = createClient();
  const { register, handleSubmit, watch } = useForm<CaptainFormPost>();
  ...
  const { data: constituencies, isLoading: constituenciesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.log('error loadig sub counties', error.message);
      }
      return data;
    },
    enabled: !!county_id,
  ...
  const { data: players, isLoading: playersLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .eq('team_id', team_id);
      if (error) {
        console.log('error loadig players', error.message);
      }
      return data;
    },
    enabled: !!team_id,
